...
const spots = [
  ...
  { name: "Bus Stop 24", lat: 19.079415349953123, lng: 72.83446727495458, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 25", lat: 19.075973866552673, lng: 72.83410312617936, category: "bus", safetyStatus: "unsafe" },
  { name: "Bus Stop 26", lat: 19.090812, lng: 72.841572, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 27", lat: 19.088121, lng: 72.845291, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 28", lat: 19.086243, lng: 72.843158, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 29", lat: 19.084566, lng: 72.838932, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 30", lat: 19.082002, lng: 72.841020, category: "bus", safetyStatus: "safe" },
  { name: "Bus Stop 31", lat: 19.078432, lng: 72.832341, category: "bus", safetyStatus: "unsafe" },
  { name: "Bus Stop 32", lat: 19.077103, lng: 72.830224, category: "bus", safetyStatus: "unsafe" },
  { name: "Bus Stop 33", lat: 19.074910, lng: 72.835610, category: "bus", safetyStatus: "unsafe" },
  { name: "Bus Stop 34", lat: 19.073801, lng: 72.833478, category: "bus", safetyStatus: "unsafe" },
  { name: "Bus Stop 35", lat: 19.071652, lng: 72.834140, category: "bus", safetyStatus: "unsafe" },
...
// Firebase endpoint
const FIREBASE_URL = "http://localhost:5000/report";

// Upload spots one by one
for (const spot of spots) {
  try {
    const response = await fetch(FIREBASE_URL, {
      ...
      body: JSON.stringify({
        location: spot.name,
        dangerLevel: spot.safetyStatus.toUpperCase(),
        category: spot.category
      }),
    });
    const data = await response.json();
    console.log(`Added: ${spot.name} → Status: ${data.status || "success"}`);
  } catch (err) {
    console.error(`Error adding ${spot.name}:`, err.message);
  }
}
